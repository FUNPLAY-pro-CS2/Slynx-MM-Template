...
namespace TemplatePlugin
{
    CAttackerInfo::CAttackerInfo(CEntityInstance* attacker)
    {
        ...
        if (!attacker ||
            (attacker->m_pEntity &&
                std::strcmp(attacker->m_pEntity->m_designerName.String(), "cs_player_controller") != 0))
            return;
        ...
        if (auto* controller = static_cast<CCSPlayerController*>(attacker))
        {
            IsWorld = false;
            IsPawn = true;
            AttackerUserId = static_cast<unsigned short>(shared::g_pEngine->GetPlayerUserId(controller->GetPlayerSlot())
                                                                          .Get());
    ...
    HitGroup_t CTakeDamageInfoEx::GetHitGroup() const
    {
        const int off = shared::g_pGameConfig->GetOffset("CTakeDamageInfo_HitGroup");
        if (off <= 0)
            return HitGroup_t::HITGROUP_INVALID;
