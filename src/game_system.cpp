...
{
    DynLibUtils::CModule libserver(Template::shared::g_pServer);

    auto result = libserver.FindPattern(Template::shared::g_pGameConfig->GetSignature("IGameSystem_InitAllSystems_pFirst"));
    if (!result)
    {
